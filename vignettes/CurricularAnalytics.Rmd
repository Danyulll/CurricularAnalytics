...
In **CurricularAnalytics** there are several ways to store and create curriculum graph objects. We can store curriculum graphs in json files or CSV files. If stored in this way, one may load their graph through the respective json and csv functions: `curriculum_graph_from_json()` or `curriculum_graph_from_csv()`. Additionally, one may create a curriculum graph by hand in R through first creating node and edge lists and then calling `curriculum_graph_from_list()`. 
...
This is the minor in its absolute simplest form. What will be of interest in this graph are which courses appear. As this is the easiest way for a general science student to obtain the minor it is likely going to be a popular pathway.
...
Next, we demonstrate how **CurricularAnalytics** can help in creating new curricula. Inventing new programs or concentrations can be quite difficult; often there are many moving pieces the curriculum designer must consider. Through its ability to visualize and quantify degree composition our package allows the user to explore potential structures and consider "what-ifs?" with greater ease.
