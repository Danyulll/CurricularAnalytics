...
In CurricularAnalytics there are several ways to store and create curriculum graph objects. We can store curriculum graphs in json files or CSV files. One may construct the curriculum graph object either by loading the graph from a stored file either using `curriculum_graph_from_json()` or `curriculum_graph_from_csv()` or by hand in R by creating a node_list and edge_list dataframe and calling `curriculum_graph_from_list()`. 

JSON files must be of the following format

- An entry called terms specifying the number of terms in the curriculum
- A list holding all course information: 
  - label: string indicating course 
  - term: integer indicating what term the course is to be taken
  - prerequisties: list holding strings with prequsitie course labels
  - corequisites: list holding strings with corequisite course labels
...
The highest delay factor courses are MATH 100, MATH 101, MATH 200, STAT 303, and STAT 403 with a a factor of 5. Perhaps alternting STAT 303 can help reduce long paths in this graph.

The courses with the largest structural complexity are MATH 100 (21), MATH 101 (19), STAT 230 (110).
...
The 2023/2024 school year saw a major overhaul of the UBCO DS. In investigating the 2022 - 2023 DS Major curriculum graphs we identified several courses of interest and requisite relationships. Let us implement some of this changes for the DS overhaul and see how our structural complexity is impacted.
...
The most central nodes are STAT 203 (159), STAT 205 (140), and MATH 101 (130). In our new curriculum, STAT 230 has been split into two new courses each tackling different aspects of the original course. STAT 203 is the more theory heavy statistics course that leads into STAT 303, which is an intermediate probability theory course, and STAT 401, a statistical inference course. STAT 205 is more applied and includes R programming which is why it serves as the prerequisite for course like DATA 311 Machine Learning and DATA 310 Applied Regression Analysis. This structure is preferable as instead of having STAT 230 on every long path through the curriculum we have two fairly central courses that split the long pathways between them.

The courses with the greatest blocking factors are MATH 100 (22), MATH 101 (20), and DATA 101 (12). MATH 100 and 101, being Calculus 1 and 2, reetain their high blockign factors. We've added DATA 101 as a prerequite to STAT 203 which explains its rise in blocking factor compared to previous graphs.

The courses with the greatest delay factors are MATH 100, MATH 101, STAT 203, STAT 205, DATA 311, DATA 315, DATA 310, STAT 400, and DATA 410 with a delay factor of 6. This is the same as previous graphs. It seems the longest pathway in our curruicla are often 6 courses.

The courses with the largest structural complexity are unsurprinsgly MATH 100 (28), MATH 101 (26), and STAT 203 (17). These 3 courses lead into every major course in the curriculum thus they contribute the most to strucual compelxity.

The key takeaway from this graph is how the splitting of STAT 230 from one realively high central course into two realtivey medium central courses has improved the flow of courses. This graphs gives us a way to visualize this.
...
The courses with the highest blocking factor are MATh 100 (22), MATH 101 (20), and DATA 101 (12). These courses also appear the same as before. It is interesting DATA 101 appears on this list again further cementing its newfound importance in this revised curriculum.
...
Next we look at courses found in the max graph not found in the min graph and vice-versa. The courses in the max graph not found in the min graph are:
...
This curriculum is based on the older 2022/20233 academic calendar therefore we see STAT 230 present in the graph. What will interest us here is what courses find themsevles as being important and counting as a course in the minor. We could for example examine how difficult it is for minor students to enter a course like DATA 410 in the max graph and consider making entry easier. STAT 230 remains the most central course in the minor graphs like this can prompt us to investigate its rtestructuing for the minor.

## Creation of the DS Math Stream

Curricular Analytics may also be a powerful tool in inventing new programs. We demonstrate this through suggesting a math stream in the DS program. Currently many math electives are available to DS students as of the 2022/2023 acadmeic calendar. What would a DS major look like if we offered a math stream requiring all math electives?
